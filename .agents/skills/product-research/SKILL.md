...
When calling Sorftime, use the live `mcp__sorftime_server` tool schema as the source of truth. Read `.agents/references/sorftime-server-tool-schema.md` before using Sorftime tools. Historical names such as `category_report`, `category_name_search`, `category_trend`, `keyword_related_words`, and `search_categories_broadly` are compatibility notes only unless exposed by the live schema. If category/market endpoints are incomplete, combine visible MCP tools and external evidence, and label the evidence limitation.
